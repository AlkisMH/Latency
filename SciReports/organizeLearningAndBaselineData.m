...
Nb = length(ii.Learn1);
if zero_flag
    fn = {'head_dir_peak_sp0','aim_dir0','peak_sp','pert','vfb','tgt_angle','ITI','acc_time','mvt_time','max_vel'};
else
    fn = {'head_dir_peak_sp','aim_dir','peak_sp','pert','vfb','tgt_angle','ITI','acc_time','mvt_time','max_vel'};
end
fn_vec = {'all_vels','all_tgt_vels','all_tgt_pos'};
    ...
    for b=1:Nb,
        ...
        for k=1:length(fn),
            ...
            if (k==1)|(k==2),
                ...
                zz = z.x.rawdat.(fn{k})(ii.Wash1{b},s); Nz(k,k2)=length(zz);
                z.x.dat.([fn{k},'_wash'])(1:Nz(k,k2),k2) = zz;
            end
        end

        for k=1:length(fn_vec),
            zz = z.x.rawdat.(fn_vec{k})(:,ii.Learn1{b},s); Nz(k,k2)=size(zz,2);
            z.x.dat.(fn_vec{k})(:,1:Nz(k,k2),k2) = zz;
            % if (k==1)|(k==2),
            %     zz = z.x.rawdat.(fn{k})(ii.Base1{b},s); Nz(k,k2)=length(zz);
            %     z.x.dat.([fn{k},'_base'])(1:Nz(k,k2),k2) = zz;
            % end
...
z.x.dat.tot_wash = z.x.dat.([fn{1},'_wash']);
z.x.dat.exp_wash = z.x.dat.([fn{2},'_wash']);
z.x.dat.imp_wash = z.x.dat.tot_wash - z.x.dat.([fn{2},'_wash']);
end