...
%xi  = z.x.dat.imp .* sign(z.x.vmr);
%xi0 = z.x.dat.imp_base .* sign(z.x.vmr);
xe  = z.x.dat.exp .* sign(z.x.vmr);
xe0 = z.x.dat.exp_base .* sign(z.x.vmr);

for m = 1:3
    i_m = find(z.x.delay==dList(m));
 %   xi(:,i_m) = removeOutliers(xi(:,i_m),nIQR);
 %   xi0(:,i_m) = removeOutliers(xi0(:,i_m),nIQR);
